 ...
 * @version 0.1.0
 */
public class TextParagraphMultipleP extends TextParagraph {
	public TextParagraphMultipleP() {
		...
		ExParValue[] dd = new ExParValue[2];
		dd[0] = new ExParValueConstant("de.pxlab.pxl.PxlColor.CYAN");
		dd[1] = new ExParValueConstant("de.pxlab.pxl.AlignmentCodes.YELLOW");
		Color.getValue().set(ExParValue.constantExParValue(dd));
	...
	protected void computeGeometry() {
		...
		int[] al = Alignment.getIntArray();
		int[] dd = Color.getIntArray();
		...
		for (int i = 0; i < n; i++) {
			...
			te.setProperties(txt2, FontFamily.getString(), FontStyle.getInt(),
					...
					(i >= al.length) ? al[al.length - 1] : al[i],
