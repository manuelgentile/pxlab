...
/**
 * DirectInputDriver: the connection to the Win32 joystick. There is only one
 * allowed, so the layout of this class is merely static.
 * 
 * History:
 * 
 * Changes since 0.1beta: - support of multiple devices addressed by the
 * <code>dev</code> index
 * 
 * 
 * @author Herkules
...
class DirectInputDriver {
	...
	static {
		try {
			...
			Log.logger
					.warning("Native library jxinput not loaded due to a SecurityException.");
		} catch (UnsatisfiedLinkError e) {
			Log.logger
					.info("Native library jxinput not loaded due to an UnsatisfiedLinkError.");
		}
	}
	...
	private final static void init() {
		...
		if (nativeinit()) {
			...
			sAxisValues = new double[devs][DirectInputDriver
					.getMaxNumberOfAxes()];
			sButtonStates = new boolean[devs][DirectInputDriver
					.getMaxNumberOfButtons()];
			sDirectionalValues = new int[devs][DirectInputDriver
					.getMaxNumberOfDirectionals()];
	...
	/**
	 * Static ctor of DirectInputDriver. No object will be created due to the
	 * static layout.
	 */
	...
	/**
	 * Shutdown the device and free all Win32 resources. It is not a good idea
	 * to access any joystick features after <code>shutdown()</code>.
	 */
