...
public class JXInputDevicePanel extends javax.swing.JPanel {
	private static final Font AXIS_SLIDER_FONT = new Font("Verdana",
			Font.PLAIN, 9);
	...
	private class AxisSlider extends JSlider {
		...
		void update() {
			...
			if (ax != this.getValue()) {
				this.setValue(ax);
				this.setToolTipText(mAxis.getName() + ": "
						+ Double.toString(mAxis.getValue()));
			}
		}
	}
	...
	private class DirectionalLabel extends JLabel {
		...
		void update() {
			...
			//
			if (dir != mCurrent) {
				this.setText(mDirectional.getName()
						+ ":  "
						+ (mDirectional.isCentered() ? "-" : Integer
								.toString(dir)));
				mCurrent = dir;
			}
		}
	...
	void initFromDevice() {
		if (null != mDev) {
			((GridLayout) mAxesPanel.getLayout()).setRows(mDev
					.getNumberOfAxes());
			...
			((GridLayout) mButtonsPanel.getLayout()).setRows(mDev
					.getNumberOfButtons());
			...
			((GridLayout) mDirectionalPanel.getLayout()).setRows(mDev
					.getNumberOfDirectionals() / 2);
			for (int i = 0; i < mDev.getMaxNumberOfDirectionals(); ++i) {
				if (null != mDev.getDirectional(i)) {
					DirectionalLabel lbl = new DirectionalLabel(
							mDev.getDirectional(i));
	...
	private void initComponents() {
		...
		mAxesPanelContainer.setBorder(javax.swing.BorderFactory
		...
		mDirectionalPanel.setBorder(javax.swing.BorderFactory
				.createBevelBorder(javax.swing.border.BevelBorder.RAISED));
		add(mDirectionalPanel, java.awt.BorderLayout.SOUTH);
		mButtonsPanel.setLayout(new java.awt.GridLayout(1, 1));
		mButtonsPanel.setBorder(javax.swing.BorderFactory
				.createBevelBorder(javax.swing.border.BevelBorder.RAISED));
