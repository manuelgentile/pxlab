...
/**
 * Handles all events and listeners. <code>JXInputEventManager</code> is layed
 * out a static singleton.
 * 
 * @author Herkules
 */
public class JXInputEventManager {
	...
	private static class axislistener {
		...
		axislistener(JXInputAxisEventListener l, Axis axis, double treshold) {
			if (axis == null)
				System.out
						.println("JXInputEventManager.axislistener() axis argument is null");
			mListener = l;
			mTreshold = treshold;
			mEvent = new JXInputAxisEvent(axis);
			if (mEvent == null)
				System.out
						.println("JXInputEventManager.axislistener() mEvent is null");
		}

		final void checkTrigger() {
			Axis a = mEvent.getAxis();
			if (a == null)
				System.out
						.println("JXInputEventManager.axislistener() Axis is null");
	...
	private static class directionallistener {
		...
		directionallistener(JXInputDirectionalEventListener l,
				Directional directional, double valuetreshold) {
			mListener = l;
		...
		final void checkTrigger() {
			...
			if (Math.abs(delta) >= mValueTreshold || Math.abs(dirdelta) > 0
					|| centeredchanged) {
				mLastValueFired = curval;
	...
	/**
	 * Query devices and fire all occuring events. <code>trigger()</code> is
	 * thought to be called by <code>JXInputManager#updateFeatures()</code>.
	 */
	public static void trigger() {
		...
		for (int i = 0; i < n; i++) {
			directionallistener l = (directionallistener) mDirectionalEventListeners
					.get(i);
			l.checkTrigger();
		}
	}
	...
	/**
	 * Set the intervall in ms that is used to check for new values of the
	 * features. Set it to <= 0 to prohibit automatic triggering. Events will
	 * then only be fired when somebody invokes
	 * <code>JXInputManager#updateFeatures()</code>.
	 */
	...
	public static void addListener(JXInputAxisEventListener l, Axis axis,
		...
		if (axis == null) {
			System.out
					.println("JXInputEventManager.addListener( / / ) axis argument is null");
			new RuntimeException().printStackTrace();
		}
		mAxisEventListeners.add(new JXInputEventManager.axislistener(l, axis,
				treshold));
	}
	...
	public static void addListener(JXInputButtonEventListener l, Button button) {
		mButtonEventListeners.add(new JXInputEventManager.buttonlistener(l,
				button));
	}
	...
	public static void addListener(JXInputDirectionalEventListener l,
			Directional directional, double valuetreshold) {
		mDirectionalEventListeners
				.add(new JXInputEventManager.directionallistener(l,
						directional, valuetreshold));
	}
	...
	public static void addListener(JXInputDirectionalEventListener l,
		...
		mDirectionalEventListeners
				.add(new JXInputEventManager.directionallistener(l,
						directional, directional.getResolution()));
	}
