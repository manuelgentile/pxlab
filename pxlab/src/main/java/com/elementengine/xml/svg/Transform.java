...
public class Transform extends AffineTransform {
	...
	void setTransform(java.lang.String transformString) {
		...
		transformString = transformString.replace((char) 0x0D, ' '); // suppress
																		// newline
		transformString = transformString.replace((char) 0x0A, ' '); // suppress
																		// newline
		...
		if (transformOffset >= 0) {
			...
			if (endParenOffset > 0) {
				transformData = transformString.substring(transformOffset + 9,
						endParenOffset + 1);
				transformValue = new double[2]; // trusting that SVG will match
												// required fields
		...
		if (transformOffset >= 0) {
			endParenOffset = transformString.indexOf(")", transformOffset);
			if (endParenOffset > 0) {
				transformData = transformString.substring(transformOffset + 5,
						endParenOffset + 1);
		...
		if (transformOffset >= 0) {
			endParenOffset = transformString.indexOf(")", transformOffset);
			if (endParenOffset > 0) {
				transformData = transformString.substring(transformOffset + 6,
						endParenOffset + 1);
				transformTokens = new StringTokenizer(transformData, ",() \t\n");
				numTransformTokens = transformTokens.countTokens();
				transformValue = new double[numTransformTokens];
				...
				if (numTransformTokens == 3) {
					this.rotate(transformValue[0] * (Math.PI / 180),
							transformValue[1], transformValue[2]);
				}
			}
		}
		...
		if (transformOffset >= 0) {
			...
			if (endParenOffset > 0) {
				transformData = transformString.substring(transformOffset + 5,
						endParenOffset + 1);
				transformValue = new double[1]; // trusting that SVG will match
												// required fields
				transformValue = Utility.getDoubles(transformData);
