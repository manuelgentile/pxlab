...
// H. I. 2006/02/21 allow double values for size parameters in SVG file 
...
class svgImage extends Canvas {
	...
	public BasicStroke bs = new BasicStroke(1.0f, BasicStroke.CAP_BUTT,
	...
	public Dimension getMinimumSize() {
		return new Dimension((int) Math.round(0.10 * svgWidth),
				(int) Math.round(0.10 * svgHeight));
	}
	...
	public void renderSVG() {
		...
		while (e.hasMoreElements()) {
			...
			System.out.println("svgImage.renderSVG() Element " + eCount + ": "
					+ elem.getTagName() + " level = " + elem.getNestLevel());
			...
			if (lastNestLevel > nestLevel) { // A group has been closed
				...
				if (nestLevel == 1) {
					...
					String grp = new String("group" + (nestLevel - 1));
					System.out.println("svgImage.renderSVG() Close group "
							+ grp);
					Group restoreGroup = (Group) groupHash.get(grp);
					globalStyle.setStyle(restoreGroup.getStyle());
					globalTransform.setTransform((AffineTransform) restoreGroup
							.getTransform());
					currentStyleLevel = nestLevel;
				}
			}
			lastNestLevel = nestLevel;
			localStyle.setStyle(globalStyle); // Apply global style by object
												// assignment
			...
			if (elem.hasAttribute("style") && !elem.getTagName().equals("g")) {
				localStyle.setStyle(globalStyle); // Apply global style by
													// object assignment
				tempBuffer = elem.getAttribute("style");
				localStyle.setStyle(tempBuffer); // add local style by String
													// based assignment
			}
			svgGraphics.setTransform((AffineTransform) globalTransform);
			if (elem.hasAttribute("transform")
					&& !elem.getTagName().equals("g")) {
			...
			if (elem.getTagName().equals("g")) {
				System.out.println("svgImage.renderSVG() New group found.");
				...
				String grp = new String("group" + nestLevel);
				System.out.println("svgImage.renderSVG() Open group " + grp);
				groupHash.put(grp, saveGroup);
				...
				if (rounded) {
					roundRect = new RoundRectangle2D.Float(fx, fy, fw, fh, frx,
							fry);
				} else {
				...
				fy = fy - fh;
				fx = fx - fw;
				Ellipse2D.Float ellipse = new Ellipse2D.Float(fx, fy, 2 * fw,
						2 * fh);
				svgGraphics.setPaint(localStyle.getFillColor());
				...
				tempBuffer = tempBuffer.replace((char) 0x0D, ' '); // suppress
																	// newline
				tempBuffer = tempBuffer.replace((char) 0x0A, ' '); // suppress
																	// newline
				...
				t = new StringTokenizer(tempBuffer,
						"MmLlCczArSsHhVvDdEeFfGgJjQqTtzZ", true);
				...
				while (t.hasMoreElements()) {
					...
					if (offset >= 0) {
						...
						pathFloat = new float[t.countTokens()]; // trusting that
																// right number
																// will be in
																// SVG
					...
					case 'l': // Relative Line to
						...
						break;
					case 'C': // Cubic Bezier
						p.curveTo(pathFloat[0], pathFloat[1], pathFloat[2],
								pathFloat[3], pathFloat[4], pathFloat[5]);
					...
					case 'c': // relative Cubic Bezier
						...
						break;
					case 'S': // Smooth Cubic
						p.curveTo(2 * currentX - lastCubicControlX, 2
								* currentY - lastQuadControlY, pathFloat[0],
								pathFloat[1], pathFloat[2], pathFloat[3]);
					...
					case 'A': // Elliptical Arc
						...
						p = arcTo(pathFloat[0], pathFloat[1], pathFloat[2],
								largeArc, sweep, pathFloat[5], pathFloat[6], p);
						break;
					...
					case 'v': // relativ vertical line
						...
						break;
					case 'Q': // quadratic
						p.quadTo(pathFloat[0], pathFloat[1], pathFloat[2],
								pathFloat[3]);
					...
					case 'T': // true type - quadratic
						p.quadTo(2 * currentX - lastQuadControlX, 2 * currentY
								- lastQuadControlY, pathFloat[0], pathFloat[1]);
						currentX = (float) p.getCurrentPoint().getX();
						currentY = (float) p.getCurrentPoint().getY();
						break;
					case 't': // relative true type - quadratic
						p.quadTo(2 * currentX - lastQuadControlX, 2 * currentY
								- lastQuadControlY, pathFloat[0] + currentX,
								pathFloat[1] + currentY);
						currentX = (float) p.getCurrentPoint().getX();
						currentY = (float) p.getCurrentPoint().getY();
						break;
	...
	public void getSvgSize() {
		...
		while (e.hasMoreElements()) {
			...
			if (elem.getTagName().equals("svg")) {
				...
				// System.out.println("svgImage.getSvgSize() w=" + tempWidth +
				// "  h=" + tempHeight);
				...
				if (noMeasure) {
					...
					// svgWidth = (short)new
					// Integer(tempWidth.trim()).intValue();
					svgWidth = (short) (Math.ceil(new Double(tempWidth)
							.doubleValue()));
				}
				if (pxOffset >= 0) {
				...
				if (inchOffset >= 0) {
					...
					svgWidth = (short) (Math.ceil(new Double(tempWidth)
							.doubleValue()));
					// svgWidth = (short)new Integer(tempWidth).intValue();
					pixelsPerInch = Toolkit.getDefaultToolkit()
							.getScreenResolution();
				...
				if (noMeasure) {
					...
					svgHeight = (short) (Math.ceil(new Double(tempHeight)
							.doubleValue()));
				}
				if (pxOffset >= 0) {
	...
	public GeneralPath arcTo(double rx, double ry, double angle,
