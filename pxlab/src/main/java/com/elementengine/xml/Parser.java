	...
	private void internalParser(String filename, String bufSize) {
		...
		try {
			while (bytesRead >= 0 && buf.ready()) {
				...
				xmlBuffer = xmlFragment + xmlBuffer; // take fragment and add
														// what is read from
														// buffer
		...
		if (xmlString.startsWith("<?") || xmlString.startsWith("<!")
			...
			if (xmlString.startsWith("</")) { // byproduct of last tag
				...
				if (!endString.equals(testString)) {
					System.out.println("Parsing error near: " + xmlString
							+ "  element number " + elementCount);
					System.exit(0);
				}
				nestLevel--;
			} else {
				...
				if (offset > 0) {
					nestLevel++;
					xmlString = xmlString.replace((char) 0x0D, ' '); // suppress
																		// newline
					xmlString = xmlString.replace((char) 0x0A, ' '); // suppress
																		// newline
					xmlString = xmlString.replace('\t', ' '); // suppress tab
					xmlString = Utility.trim(xmlString); // added March 8, 2003
					offset = xmlString.indexOf(" "); // the get tag name based
														// on SPACE
					if (offset > 0) {
						tagName = xmlString.substring(1, offset);
						e = new Element(tagName, xmlString.substring(offset),
								nestLevel);
						System.out.println("Parser.parseElements(): " + tagName
								+ " level = " + nestLevel);
						elements.add(e);
						nestLevel--;
						elementCount++;
					...
					if (offset > 0) {
						...
						e = new Element(tagName,
								xmlString.substring(offset + 1), nestLevel);
						elements.add(e);
						System.out.println("Parser.parseElements(): " + tagName
								+ " level = " + nestLevel);
						elementCount++;
